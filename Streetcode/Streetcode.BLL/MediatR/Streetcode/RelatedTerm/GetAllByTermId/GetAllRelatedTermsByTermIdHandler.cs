...
public record GetAllRelatedTermsByTermIdHandler
    : IRequestHandler<GetAllRelatedTermsByTermIdQuery, Result<IEnumerable<RelatedTermDTO>>>
{
    ...
    public async Task<Result<IEnumerable<RelatedTermDTO>>> Handle(GetAllRelatedTermsByTermIdQuery request, CancellationToken cancellationToken)
    {
        ...
        var relatedTermsDtos = _mapper.Map<IEnumerable<RelatedTermDTO>>(relatedTerms);

        if (relatedTermsDtos is null)
        {
        ...
        return Result.Ok(relatedTermsDtos);
    }
}
